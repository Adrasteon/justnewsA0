...
while true; do
  ...
  if systemctl --user is-active --quiet vllm.service; then
    echo "vllm: active" >> "$LOG"
  else
    echo "vllm: inactive" >> "$LOG"
    systemctl --user status vllm.service --no-pager --lines=3 >> "$LOG" 2>&1 || true
  fi
  echo "----" >> "$LOG"
  sleep "$INTERVAL"
done
