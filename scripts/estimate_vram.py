...
def estimate_vram():
    ...
    print(f"1. TOTAL CONCURRENT LOAD (All Models in VRAM):")
    print(f"   Total Weights:   {total_weights:.2f} GB")
    print(f"   Combined Ctx:    {total_ctx:.2f} GB")
    print(f"   Grand Total:     {grand_total:.2f} GB")
    print(f"   Available:       24.00 GB")
    print(f"   Headroom:        {24.00 - grand_total:.2f} GB")
    
    if grand_total < 24.0:
        print(f"   Status:          ✅ FITS (Simultaneous Residency Possible!)")
    else:
        print(f"   Status:          ❌ OOM (Overflow)")
